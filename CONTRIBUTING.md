# Contributing to arrivd

## Getting Started

```bash
git clone https://github.com/arrivd/arrivd.git
cd arrivd
bun install
bun run build
bun run test
...
|---------|------|-------------|
| `@arrivd/core` | `packages/core` | Content protection engine (obfuscation, honeypots, clipboard, devtools) |
| `@arrivd/react` | `packages/react` | React components (`ObscrdProvider`, `ProtectedText`, `ProtectedEmail`) |
| `@arrivd/robots` | `packages/robots` | AI crawler blocking (robots.txt generation, middleware) |
...
- Steps to reproduce
- arrivd version and runtime (Bun/Node)

## Security Vulnerabilities

Do not open public issues for security vulnerabilities. Email [security@arrivd.dev](mailto:security@arrivd.dev) instead. See `SECURITY.md` for details.
